...
float softshadow( in vec3 landPoint, in vec3 lightVector, float mint, float maxt, float iterations ){
    ...
    for( float t = mint; t < maxt; ){
        float nextDist = Building1Distance(landPoint * 10 + lightVector * t);
        if( nextDist < 0.001 )
            return 0.0;
...
vec3 rayCast(in vec3 position, in vec3 direction, inout vec3 hitColor){
  ...
  for (i = 0; i < MAX_STEPS ; ++i){
    ...
    //lastJazzBuilding = CubeRepetition(position, vec3(20.0, 0.0, 20.0));
...
void main(void)
{
  ...
  vec3 lightpos = vec3(0.0, 5.0, (fuffaTime*0.1) + 10.0 );
  ...
  float penumbra = softshadow(landingPixel, normalize(lightpos - landingPixel), 0.001, 10, 8);

  landingPixel.z -= fuffaTime*0.1;
  ...
  hitColor = mix(vec3(0.35), hitColor,(penumbra));
