...
float edgeDetection(in vec2 uncoords){
  ...
  return clamp((ddx + ddy - 0.01)*100.0,0.0,1.0)/(depth0 * 5);
}
...
void main (void){
    ...
    if (gl_FragCoord.x < (windowSize.x * 0.5) ) {
      ...
      out_Color = vec4(texture2D(normalsTexture, vec2(gl_FragCoord.x/windowSize.x, gl_FragCoord.y/windowSize.y)).aaa, 1.0);
    }
